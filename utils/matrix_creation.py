...
def filter_sparse_data(
    df: pd.DataFrame, min_ratings_per_user: int = 20, min_ratings_per_movie: int = 50
) -> pd.DataFrame:
    """
    Filters out users and movies with too few interactions to improve model quality.
    """
    logger.info(
        f"🧹 Filtering sparse data (Min Ratings: User={min_ratings_per_user}, Movie={min_ratings_per_movie})"
    )
    try:
        # Filter Movies
