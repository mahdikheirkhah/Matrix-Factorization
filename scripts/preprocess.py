...
def run_preprocessing():
    try:
        ...
        ghost_ids = ratings[~ratings["movie_id"].isin(movies_df["movie_id"])][
            "movie_id"
        ].unique()
        if len(ghost_ids) > 0:
            logger.warning(
                f"👻 Found {len(ghost_ids)} Movie IDs with no metadata! Purging..."
            )
        ...
        if not duplicates.empty:
            logger.info(
                f"🔍 Found {len(duplicates)} movies with duplicate titles (Inconsistencies):"
            )
        ...
        temp_merged = ratings.merge(
            movies_df[["movie_id", "release_year"]], on="movie_id"
        )
        temp_merged["rating_year"] = pd.to_datetime(
            temp_merged["timestamp"], unit="s"
        ).dt.year

        # Find ratings that happened BEFORE the release year
        time_travelers = temp_merged[
            temp_merged["rating_year"] < temp_merged["release_year"]
        ]

        if len(time_travelers) > 0:
            logger.warning(
                f"⏳ Found {len(time_travelers)} ratings predating movie release! Purging..."
            )
            # Keep only valid temporal ratings
            valid_indices = temp_merged[
                temp_merged["rating_year"] >= temp_merged["release_year"]
            ].index
        ...
        clean_ratings = filter_sparse_data(
            ratings, min_ratings_per_user=50, min_ratings_per_movie=150
        )
