...
def save_prediction_plot(actuals, preds, model_name, filename):
    ...
    plt.scatter(x_jittered, preds, alpha=0.5, color='#3498db' if model_name == "PMF" else '#e67e22', edgecolors='w')
...
def generate_all_plots(test_df, matrix, user_means, pmf_factors, svd_preds):
    ...
    print("⚠️ Note: Only a sample of 10,000 test ratings is plotted for clarity. but metrics are calculated on the full test set. with size:" + str(len(test_df)))
    ...
    save_prediction_plot(svd_actuals, svd_preds_list, "SVD", "predicted_vs_actual_svd.png")
    
    save_5x5_confusion_matrix(pmf_actuals, pmf_preds, "PMF", "confusion_matrix_pmf.png")
    save_5x5_confusion_matrix(svd_actuals, svd_preds_list, "SVD", "confusion_matrix_svd.png")
...
def analyze_user_recommendations(U, V, matrix, movies_df, test_df, num_users=10):
    ...
    for u_id in test_users:
        ...
        print(f"👤 User {u_id:4} | 🎬 Rec: {title[:30]:30} | 🔑 Factor {top_factor:2} | ✨ Score: {contributions[top_factor]:.4f}")
