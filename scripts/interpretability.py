...
def save_comparative_confusion_matrix(actuals: List[float], preds: List[float], model_name: str, filename: str) -> None:
...
def plot_prediction_drift(test_df: pd.DataFrame, pmf_preds: List[float], model_name: str):
    """
...
def run_interpretability_analysis():
    ...
    try:
        ...
        # 3. Generate Reports
        # Confusion Matrices
        save_comparative_confusion_matrix(actuals, pmf_final_preds, "PMF Model", "pmf_confusion_matrix")
        save_comparative_confusion_matrix(actuals, svd_final_preds, "SVD Model", "svd_confusion_matrix")
        
        # Drift Charts (The replacement for latent factors)
        plot_prediction_drift(test_df[test_df['user_id'].isin(user_map.keys())].head(len(actuals)), pmf_final_preds, "PMF")
        plot_prediction_drift(test_df[test_df['user_id'].isin(user_map.keys())].head(len(actuals)), svd_final_preds, "SVD")
