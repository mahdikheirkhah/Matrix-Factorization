...
with col1:
    ...
    if user_input in user_map:
        st.success(
            f"✅ **User ID {user_input} was in the training set.** (Showing Personalized PMF Predictions)"
        )

    # 2. Check Test Set
    elif user_input in test_users:
        st.info(
            f"ℹ️ **User ID {user_input} found in the test set.** (Showing Global Fallback Predictions)"
        )

    # 3. Check Original Dataset (Sparsity Filtered)
    elif user_input in all_users:
        st.warning(
            f"⚠️ **Cold Start:** User {user_input} removed during preprocessing due to sparsity. (Showing Global Fallback Predictions)"
        )
