...
with col1:
    ...
    # 2. Check test_ratings.csv
        ...
        st.info(f"ℹ️ **Cold Start:** User {user_input} not in training data, but found in Test Set (`test_ratings.csv`).")
        st.caption("Showing Global Fallback Predictions.")
        
    # 3. Check users.dat
    elif user_input in all_users:
        st.warning(f"⚠️ **Sparsity Filtered:** User {user_input} exists in `users.dat` but was removed during preprocessing.")
        st.caption("Showing Global Fallback Predictions.")
